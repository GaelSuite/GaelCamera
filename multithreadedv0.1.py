...
def display_ar_overlays(frame, center, speed, acceleration, total_distance, robot_id="R001"):
    ...
    cv2.putText(frame, f"Speed: {speed:.2f} ft/s", (center[0] + 20, center[1] - 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
    cv2.putText(frame, f"Acceleration: {acceleration:.2f} ft/s^2", (center[0] + 20, center[1] - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
    cv2.putText(frame, f"Total Distance: {total_distance:.2f} ft", (center[0] + 20, center[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
    cv2.putText(frame, f"ID: {robot_id}", (center[0] + 20, center[1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, text_color, 2)
...
def tracking_thread(matrix, cap, temp_frame):
    global heatmap, roi_being_selected, roi_selected, tracker_initialized, x_cur, y_cur, top_left_pt, bottom_right_pt
                ...
                if len(past_points) >= 2:
                    ...
                    if speed_value > speed_threshold:
                        ...
                        for i in range(len(trajectory) - 1):
                            cv2.line(frame, tuple(map(int, trajectory[i])), tuple(map(int, trajectory[i+1])), get_speed_color(center, trajectory[-1]), 2)
                        cv2.arrowedLine(frame, tuple(map(int, trajectory[-2])), tuple(map(int, trajectory[-1])), get_speed_color(center, trajectory[-1]), 2, tipLength=0.5)
...
def main():
    global heatmap, roi_being_selected, roi_selected, tracker_initialized, x_cur, y_cur

    cap = cv2.VideoCapture('match.mp4')
    ret, temp_frame = cap.read()

    cv2.namedWindow('Select Field Corners')
    cv2.setMouseCallback('Select Field Corners', get_field_corners, temp_frame)
    ...
    tracker_thread = threading.Thread(target=tracking_thread, args=(matrix, cap, temp_frame))
